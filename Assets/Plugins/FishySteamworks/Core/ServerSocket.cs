...
namespace FishySteamworks.Server
{
    public class ServerSocket : CommonSocket
    {
        ...
        {
            foreach (var connection in new List<KeyValuePair<HSteamNetConnection, int>>(_steamConnections.First))
                StopConnection(connection.Value, connection.Key);
        ...
        {
            if (args.m_info.m_hListenSocket != _socket || _socket == HSteamListenSocket.Invalid) return;
