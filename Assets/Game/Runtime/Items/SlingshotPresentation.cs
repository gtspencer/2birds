...
namespace TwoBirds
{
    ...
    public sealed class SlingshotPresentation : MonoBehaviour
    {
            ...
            Pose? palm, ItemPalmContact contact, Vector3 scale)
        {
            ...
            {
                recoilDraw = draw;
                releaseLocal = previous == ItemActionState.Charging ? loadedLocal : ToLocal(frame, held, scale);
            }
            ...
            if (state == ItemActionState.Recovering)
            {
                ...
                float settle = Mathf.Min(SettleSeconds, recovery * 0.7f);
                Center = Vector3.Lerp(loose, held, Mathf.SmoothStep(0f, 1f,
                    Mathf.InverseLerp(settle, Mathf.Max(settle, recovery), elapsed)));
                slack = 1f - recoil * recoilDraw;
            }
            LoadedPebble.position = Center;
            if (Loaded) { HasLoadedPose = true; DepartureCenter = Center; loadedLocal = ToLocal(frame, Center, scale); }
        ...
        private static Vector3 ToLocal(Pose frame, Vector3 world, Vector3 scale)
        {
            Vector3 local = Quaternion.Inverse(frame.rotation) * (world - frame.position);
