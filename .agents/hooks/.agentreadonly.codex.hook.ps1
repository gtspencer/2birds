...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($raw)) {
        Block-Codex 'The .agentreadonly guard received no hook input.'
    }
    $data = $raw | ConvertFrom-Json -ErrorAction Stop
} catch {
    Block-Codex ("The .agentreadonly guard could not parse hook input: " + $_.Exception.Message)
}
...
$isShellTool = $toolName -eq 'Bash'
$isWriteTool = $toolName -eq 'apply_patch' -or $toolName -match '(?i)(edit|write|patch|create|delete|remove|move|rename|update|insert|replace|save|upload)'
...
while (-not (Test-Path -LiteralPath (Join-Path $projectRoot '.git'))) {
    ...
    if ([string]::IsNullOrEmpty($projectRoot)) {
        Block-Codex 'The .agentreadonly guard could not determine the Git repository root.'
    }
}

$agentreadonly = Join-Path $projectRoot '.agentreadonly'
if (-not (Test-Path -LiteralPath $agentreadonly -PathType Leaf)) {
    Block-Codex "Required policy file '$agentreadonly' is missing."
}
...
try {
    $ignoreCache = Join-Path ([System.IO.Path]::GetTempPath()) 'codex-agentreadonly-check'
    ...
    if (-not (Test-Path -LiteralPath (Join-Path $ignoreGitDir 'HEAD') -PathType Leaf)) {
        ...
        if ($LASTEXITCODE -ne 0) {
            Block-Codex 'The .agentreadonly guard could not initialize its temporary Git matcher.'
        }
    }
} catch {
    Block-Codex ("The .agentreadonly guard could not initialize its matcher: " + $_.Exception.Message)
}
...
try {
    ...
    Block-Codex ("The .agentreadonly guard failed while matching paths: " + $_.Exception.Message)
}

if ($exitCode -eq 1) { exit 0 }
if ($exitCode -ne 0 -or $ignored.Count -eq 0) {
    Block-Codex 'The .agentreadonly matcher failed.'
}

$hit = [string]$ignored[0]
$candidate = if ($probeToCandidate.ContainsKey($hit)) { $probeToCandidate[$hit] } else { $hit }
Block-Codex "Path '$candidate' is read-only by .agentreadonly policy."
